        ...
        for i_episode in range(num_episodes):
            ...
            for t in count():
                ...
                if advice_prob > random.random():
                    done1, reward1 = agent_list[1].train(memory_list[2], agent_list[2])
                    done2, reward2 = agent_list[2].train(memory_list[1], agent_list[1])
                else:
                    done1, reward1 = agent_list[1].train(memory_list[1], agent_list[1])
                    done2, reward2 = agent_list[2].train(memory_list[2], agent_list[2])
...
class DQNagent():
    ...
    def optimize_model(self, agent, memory):
        ...
        non_final_next_states = torch.cat([s for s in batch.next_state
                                                    if s is not None])   # (128, 3)
        state_batch = torch.cat(batch.state)
        action_batch = torch.cat(batch.action)
        reward_batch = torch.cat(batch.reward)

        state_action_values = self.policy_net(state_batch).gather(1, action_batch) # (128, 1)
